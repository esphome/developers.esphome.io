...
1. **Conditional components** - Components that only need to run under specific conditions
1. **One-time operations** - Components that complete their work and no longer need updates
1. **Power optimization** - Reducing CPU usage for battery-powered devices
...
1. **Clear state before disabling** - Ensure the component is in a clean state before disabling the loop

1. **Document loop dependencies** - If your component requires continuous looping, document why

1. **Prefer event-driven design** - Use interrupts, callbacks, or timers instead of polling when possible

1. **Test edge cases** - Verify behavior when rapidly enabling/disabling the loop
...
1. Check if your component is in the loop state:

   ```cpp
   bool is_looping = this->is_in_loop_state();
   ```

1. Use the Logger component's async support as a reference implementation
