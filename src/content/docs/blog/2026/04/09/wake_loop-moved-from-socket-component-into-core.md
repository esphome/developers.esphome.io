---
title: "wake_loop Moved from Socket Component into Core"
date: 2026-04-09
authors: bdraco
---
...
| Zephyr (nRF52) | `k_sem_give()` (ISR-safe) / `k_sem_take()` |
...
1. **External components using `#ifdef USE_WAKE_LOOP_THREADSAFE`** — remove the guard
1. **External components with socket in `AUTO_LOAD` only for wake** — remove it
1. **External components checking `USE_SOCKET_SELECT_SUPPORT`** — use `USE_HOST` instead
