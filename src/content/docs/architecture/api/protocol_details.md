---
title: "ESPHome API Protocol Details"
---
...
1. **Plaintext Protocol**: Unencrypted communication with variable-length encoding
...
The Noise protocol provides encrypted, authenticated communication using the [Noise NNpsk0 handshake pattern](https://noiseprotocol.org/noise.html#interactive-handshake-patterns-fundamental).
...
1. **CLIENT_HELLO**: Waiting for client hello message

1. **SERVER_HELLO**: Sending server hello with device info

1. **HANDSHAKE**: Performing Noise NNpsk0 handshake exchange

1. **DATA**: Handshake complete, ready for encrypted data exchange

1. **CLOSED**: Connection closed normally

1. **FAILED**: Error occurred, connection failed

1. **EXPLICIT_REJECT**: Temporary state for sending handshake rejection
...
1. **HANDSHAKE phase**: Authentication failures, MAC failures, protocol errors
...
1. **DATA**: Ready for data exchange (no handshake required)
