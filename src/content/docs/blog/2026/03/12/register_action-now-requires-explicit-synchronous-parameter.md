---
title: "register_action Now Requires Explicit synchronous= Parameter"
date: 2026-03-12
authors: bdraco
---
...
1. The action stores args and calls `play_next_()` from a **`Component::loop()`** override (e.g. `WaitUntilAction`)
1. The action registers an **event callback** and calls `play_next_()` from it (e.g. `BLEClientWriteAction` calling from `gattc_event_handler`)
