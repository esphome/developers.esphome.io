...
- `loop()`: This method is called at each iteration of ESPHome's main application loop. The loop runs approximately
  ...
  their own tasks. Components can dynamically control their participation in the main loop (see [Component Loop Control](/architecture/components/advanced#component-loop-control) in the Advanced Topics page).
...
- `get_setup_priority()`: This method is called to determine the component's setup priority. This is used specifically
  ...
  initialized *only after* (I2C) busses are initialized. See `setup_priority` in `esphome/core/component.h` for
...
1. `on_shutdown()`: Called to initiate shutdown (send disconnect messages, stop accepting new connections)
1. `teardown()`: Called repeatedly to gracefully close connections and flush buffers. Returns `true` when complete or `false` if more time is needed
1. `on_powerdown()`: Called after all teardowns complete to power down hardware

**Forced reboot** (used for crashes, watchdog resets, or `App.reboot()`):

1. `on_shutdown()`: Called to attempt minimal cleanup
1. System restarts immediately (no teardown or powerdown)
...
For more advanced component development topics, including Component Loop Control which allows components to dynamically enable or disable their participation in the main loop for performance optimization, see the [Advanced Topics](/architecture/components/advanced) page.
