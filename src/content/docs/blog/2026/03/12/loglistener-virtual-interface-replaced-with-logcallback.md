---
title: "LogListener Virtual Interface Replaced with LogCallback"
date: 2026-03-12
authors: bdraco
---
...
1. Replace `add_log_listener(this)` with `add_log_callback(this, lambda)`
1. The lambda must be non-capturing (use the `void *self` parameter instead)
1. Remove `override` from `on_log()` — it's now a regular method
