...
1. **Fixed sizes with vector-like API**: Use `StaticVector` from `esphome/core/helpers.h` for compile-time fixed
...
1. **Runtime-known sizes**: Use `FixedVector` from `esphome/core/helpers.h` when the size is only known at runtime.
...
1. **Small datasets (1-16 elements)**: Use `std::vector` or `std::array` with simple structs instead of
...
1. **Avoid `std::deque`**: It allocates in 512-byte blocks regardless of element size, guaranteeing at least 512 bytes
   of RAM usage immediately. This is a major source of crashes on memory-constrained devices.

1. **Byte buffers**: Avoid `std::vector<uint8_t>` unless the buffer needs to grow. Use `std::unique_ptr<uint8_t[]>`.
...
1. Set the define from the component's `to_code()` function only when the user actually configures the feature, e.g.
...
- ESPHome uses a unified formatting tool for all source files (but this tool can be difficult to install).
  When creating a new PR in GitHub, be sure to check the [GitHub Actions](/contributing/submitting-your-work#automated-checks)
...
[submitting your work for integration into ESPHome](/contributing/submitting-your-work).
...
1. **Justified** with clear reasoning for why the change is necessary
1. **Accompanied** by deprecation warnings when possible (for gradual migration)
...
1. **Add the deprecation warning** using compile-time warnings or runtime logs (when possible; see compatibility
   window note), only after step 1 is complete and the codebase is clean.
1. **Maintain the old behavior** alongside the new for 6 months when possible (note: for C++ changes, maintaining
   backward compatibility is not always possible, especially for signature changes or refactorings).
1. **Document the migration path** in the PR description (which generates release notes) and code comments.

> [!NOTE]
> **Should I mark something deprecated in one PR and migrate callers later?**
>
...
| --- | --- | --- |
| *(unset, the default)* | Render the field on the editor's main form. | The field is normal config — the user genuinely wants to see it. |
...
`YAML_ONLY` > `ADVANCED` > *unset*. The single-axis shape encodes "yaml-only is strictly stronger
than advanced" at the type level — there's no way to ask for both at once, and no way to set a
contradictory state.

> [!NOTE]
> **Visibility kwarg in practice**
>
> ```python
...
1. **Maintain backward compatibility** for 6 months when possible
1. **Document the migration path** clearly in warnings and the PR description (which generates release notes)
1. **Update all examples** in esphome.io to use the new configuration format
...
1. **Justification** - Why the breaking change is necessary (RAM savings, architectural improvement, etc.)
1. **Breaking Changes Section** - Clear statement of what will break and when
1. **Migration Guide** - Concrete before/after examples for each affected use case:
   - YAML lambda examples
   - C++ external component examples
   - Common patterns (callbacks, logging, comparisons, etc.)
1. **Timeline** - Specific version when deprecated code will be removed
1. **Backward Compatibility** - What still works during the deprecation period
1. **Components Updated** - List of components already migrated (if applicable)
